    ...
    Public Function HANY(ByVal HANY1 As Decimal, ByVal HANY2 As String) As String
        ...
        Select Case HANY2
            Case "IRAQ"
                ...
                WORDPS = VPS
                If WORDINTEGER <> "" And (VPS <= 2) Then HANY = WORDINTEGER & LE & " و " & WORDPS & P & "لاغير "
                If WORDINTEGER <> "" And (VPS >= 3 And VPS <= 9) Then HANY = WORDINTEGER & LE & " و " & WORDPS & PS & "لاغير "
                If WORDINTEGER <> "" And (VPS > 9) Then HANY = WORDINTEGER & LE & " و " & WORDPS & P & "لاغير "
                If WORDINTEGER = "" And (VPS <= 2) Then HANY = WORDPS & P & "لاغير "
                If WORDINTEGER = "" And (VPS >= 3 And VPS <= 9) Then HANY = WORDPS & PS & "لاغير "
                If WORDINTEGER = "" And VPS > 9 Then HANY = WORDPS & P & "لاغير "
                If WORDINTEGER = "" And VPS = 0 Then HANY = ""
                If WORDINTEGER <> "" And VPS = 0 Then HANY = WORDINTEGER & LE & "لاغير "
            Case "USA"
                DOLLAR = " دولار "
                SENT = " سنتاً "
                SENTS = "سنتات"
                ...
                WORDINTEGER = AmountWord(V)
                WORDPS = AmountWord(VPS)
                If WORDINTEGER <> "" And (VPS <= 2) Then HANY = WORDINTEGER & DOLLAR & " و " & WORDPS & SENT & "لاغير "
                If WORDINTEGER <> "" And (VPS >= 3 And VPS <= 9) Then HANY = WORDINTEGER & DOLLAR & " و " & WORDPS & " " & SENTS & " " & "لاغير "
                If WORDINTEGER <> "" And (VPS > 9) Then HANY = WORDINTEGER & DOLLAR & " و " & WORDPS & SENT & "لاغير "
                If WORDINTEGER = "" And (VPS <= 2) Then HANY = WORDPS & SENT & "لاغير "
                If WORDINTEGER = "" And (VPS >= 3 And VPS <= 9) Then HANY = WORDPS & " " & SENTS & " " & "لاغير "
                If WORDINTEGER = "" And VPS > 9 Then HANY = WORDPS & SENT & "لاغير "
                If WORDINTEGER = "" And VPS = 0 Then HANY = ""
                If WORDINTEGER <> "" And VPS = 0 Then HANY = WORDINTEGER & DOLLAR & "لاغير "
            Case "WEIGHT"
                TON = " طن "
                KG = " كيلو جرام "
                KGS = "كيلو جرامات"
                ...
                WORDINTEGER = AmountWord(V)
                WORDPS = AmountWord(VPS)
                If WORDINTEGER <> "" And (VPS <= 2) Then HANY = WORDINTEGER & TON & " و " & WORDPS & KG & "لاغير "
                If WORDINTEGER <> "" And (VPS >= 3 And VPS <= 9) Then HANY = WORDINTEGER & TON & " و " & WORDPS & KGS & "لاغير "
                If WORDINTEGER <> "" And (VPS > 9) Then HANY = WORDINTEGER & TON & " و " & WORDPS & KG & "لاغير "
                If WORDINTEGER = "" And (VPS <= 2) Then HANY = WORDPS & KG & "لاغير "
                If WORDINTEGER = "" And (VPS >= 3 And VPS <= 9) Then HANY = WORDPS & KGS & "لاغير "
                If WORDINTEGER = "" And VPS > 9 Then HANY = WORDPS & KG & "لاغير "
                If WORDINTEGER = "" And VPS = 0 Then HANY = ""
                If WORDINTEGER <> "" And VPS = 0 Then HANY = WORDINTEGER & TON & "لاغير "
        End Select
    End Function
